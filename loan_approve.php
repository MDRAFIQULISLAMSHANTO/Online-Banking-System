...
if (isset($_POST['submit'])) {
    $money = $_POST['money'];
    $id = $_GET['loanid'];
    $select = "SELECT * from loan_request where loanid= '$id' ";
    $query = mysqli_query($con, $select);
    $num = mysqli_num_rows($query);
    $res = mysqli_fetch_array($query);
    $cid = $res['cid'];
    $status = 1;
    $queryapp = "UPDATE `loan_request` SET `status`='$status' where loanid=$id "; #update status
    $queryapp = mysqli_query($con,$queryapp);



    $select = "SELECT amount From cus_deposit Where id ='$cid'";
    ...
    if ($present > 0) {
        ...
        $total = $deposit + $money;
        $querys = "UPDATE cus_deposit Set amount = '$total' where id='$cid'";
...
$sql = "INSERT INTO cus_deposit (id,amount)VALUES ($cid, $money)";
        ...
        <div class="row d-flex  justify-content-center align-items-center">
            <div class="col-lg-8">
                ...
                <form action=" " method="post">
                    <h1 class="heading text-center">Deposit Money</h1>
